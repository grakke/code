...
public class Package {
    ...
    private int goodCount = 5; // 物品个数
    private int maxWeight = 9; // 背包承受的最大重量

    private boolean[][] memo = new boolean[5][10]; // 备忘录，默认值false

    // 回溯算法实现。注意：把输入变量都定义成成员变量
    public void f(int i, int cw) { // 调用f(0, 0)
        if (cw == maxWeight || i == goodCount) { // cw==w表示装满了，i==n表示物品都考察完了
            if (cw > maxW) maxW = cw;
            return;
        }
    ...
    public void f(int i, int cw, int cv) { // 调用f(0, 0, 0)
        ...
        if (cw + weights[i] <= maxWeight) {
            f(i + 1, cw + weights[i], cv + value[i]); // 选择装第i个物品
        }
    }
