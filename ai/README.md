...
* 张量 tensor
  * 深度
* 梯度下降算法
  * 梯度:是一个向量(矢量),表示某一函数在该点处的方向导数沿着该方向取得最大值
...
* 监督学习：回归 Supervised Learning
  ...
  3. 线性回归实现与应用
     ...
     2. 最小二乘法及代数求解 假定是一元方程，得出平方损失函数，用最小二乘法代数求解
     3. 最小二乘法的矩阵求解
        1. 在小数据集下二者运算效率接近。当面对十万或百万规模的数据时，矩阵计算的效率会高很多
     4. scikit-learn 实现 `sklearn.linear_model.LinearRegression(fit_intercept=True, normalize=False, copy_X=True, n_jobs=1)`
        * fit_intercept: 默认为 True，计算截距项。
        * normalize: 默认为 False，不针对数据进行标准化处理。
        * copy_X: 默认为 True，即使用数据的副本进行操作，防止影响原数据
        * n_jobs: 计算时的作业数量。默认为 1，若为 -1 则使用全部 CPU 参与运算
     * 一个机器学习过程往往包含训练和预测两部分，训练好的模型用于对未知数据的预测。训练模型的过程实际上是应用机器学习算法解决问题的过程。通常会定义一个损失函数（平方损失函数），并使用一种数学优化方法（最小二乘法）去求解该损失函数的最优解。这个思想将始终贯穿于机器学习之中
  4. 北京市住房价格预测
  5. 多项式回归实现与应用
  6. 比特币价格预测及绘图
  7. 岭回归和 LASSO 回归实现
